...
- Политика: сценарий `totp` | `express_push` | `express_push_then_totp`, `push_wait_seconds`
- Push: Approve → Accept; **Deny → Reject** (без TOTP); таймаут → TOTP только в сценарии then_totp
- Alembic **009**; Telegram в сценариях не участвует (отложено)
- UI: «Выпустить код» — модалка по центру

## 2026-08-25 (МСК) — модалка «Выпустить код»

- «Выпустить код» показывает QR/секрет в центре экрана (как «Настроить 2FA»), ошибки в UI; раньше панель была внизу списка пользователей
